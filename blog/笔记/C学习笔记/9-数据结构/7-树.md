...
### 性质1 非空二叉树的叶子结点等于双分支节点加1 ANKI DONE 
...
### 性质2 二叉数的第i层最多有 $2^{i-1}$ 个结点 ＡKNI DONE

结点最多为满二叉树,即一个以1为首项,公比为2的等比数列, $2^{i-1}$

### 性质3 高度/深度为k的二叉树最多有 $2^k-1$ 个结点 ANKI DONE

根据等比数列前k项求和公式,同时也是满二叉树的个数

### 性质4 具有n个结点的完全二叉树的深度/高度 ANKI DONE
...
int getDepth(BtNode *p) {
    ...
    if (p==NULL) {return 0;}
        ...
        Ldepth = getDepath(p->lchild);
        rdepth = getDepath(p->rchild);
        return (ldepth>rdepth ? ldepth : rdepth)+1;
    }
}
```

## 在二叉树中求位于先序序列中第k位置的结点值 ANKI DONE

```c
int n = 0; //定义全局变量n用于结点计数
...
```c
void search(BtNode *t, BtNode *outputNode, int key) {
    if (t!=NULL) {
        if (t->data==key) {
            outputNode = t;
        } else {
            search(t->lchild, outputNode, key);
            search(t->lchild, outputNode, key);
        }
    }
}
